const limits = {
  ...
  MAX_CONCURRENT_PROCESSCOMMITS: 100, // this influences # of http requests, so lower can be faster
  ...
  USER_DETAILS_MAX_AGE_MS: 4 * 60 * 60 * 1000, // how long do cached user details live - higher is better, but can sometimes lead to stale results (cache is purged when events are emitted, so this is generally safe)
}
