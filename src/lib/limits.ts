const limits = {
  ...
  MAX_CONCURRENT_PROCESSCOMMITS: 1024, // this influences # of http requests, so lower can be faster
  ...
  MOIZED_FETCH_MAX_AGE_MS: 15 * 30 * 1000, // default memoization when using fetch()
  ...
  USER_DETAILS_MIN_AGE_MS: 30 * 60 * 1000, // how long do cached user details live - higher is better, but can sometimes lead to stale results (cache is purged when events are emitted, so this is generally safe)
  USER_DETAILS_MAX_SIZE: 10000,
  AUTHOR_FEED_MIN_AGE_MS: 30 * 60 * 1000, // as above for author feed, resets on post
