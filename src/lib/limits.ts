const limits = {
  ...
  MAX_PENDING_INSERTS_WAIT_MS: 3 * 60 * 1000, // the maximum amount of time between inserting pending label events
  MAX_PENDING_INSERTS: 500, // the maximum number of label pending events before writing to the db
  ...
  REGULAR_POST_STDEV_MS: 3 * 1000, // the standard deviation required for a post to be considered periodic (rapidposts)
